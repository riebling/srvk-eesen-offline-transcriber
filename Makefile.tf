...
#	sox $^ -c 1 build/audio/base/$*.wav rate -v $(sample_rate)
	lame --resample $(sample_rate) --decode $^ build/audio/base/$*.wavi
	sox build/audio/base/$*.wavi -c 1 build/audio/base/$*.wav rate -v $(sample_rate)
